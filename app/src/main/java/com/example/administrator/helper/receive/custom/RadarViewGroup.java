...
public class RadarViewGroup extends ViewGroup implements RadarView.IScanningListener {
    ...
    private SparseArray<Task> mDatas;//数据源
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        //首先放置雷达扫描图
        View view = findViewById(R.id.abc);
        if (view != null) {
     ...
     * @param mDatas
     */
    public void setDatas(SparseArray<Task> mDatas) {
        this.mDatas = mDatas;
        ...
        for (int j = 0; j < dataLength; j++) {
            Task item = mDatas.get(j);
            if (item.getSendUser().getDistance() < min) {
              min = item.getSendUser().getDistance();
                minItemPosition = j;
            }
            if (item.getSendUser().getDistance() > max) {
                max = item.getSendUser().getDistance();
           }
        ...
        for (int i = 0; i < dataLength; i++) {
            ...
            circleView.setProportion((mDatas.get(i).getSendUser().getDistance() / max + 0.6f) * 0.52f);
